...
* **IncludeFilter**
If set, it is an additional filter for the files being watched. This can minimize your reloads. For example, if you want to exclude `_autogenerated.cshtml` to cause refreshes, you might want to set the filter the following way:

```csharp
services.AddLiveReload(cfg => {
    cfg.IncludeFilter = path => !path.Contains("_autogenerated");
})
```
