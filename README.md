...
This filter allows you programmatic control over whether a file should auto-refresh or not. This allows control beyond the basic file extension mapping, and lets you explicitly handle special use cases where you might not want to refresh the page when a certain file or files are updated. This can minimize your reloads especially if you are dealing with many generated files for external tools like WebPack. 
  ...
  For example, if you want to exclude `_autogenerated.cshtml` to cause refreshes, you might want to set the filter the following way:
...
Adds a configuration `Func<string, FileInclusionModes>` hook to allow explicitly specifying whether files should be refreshed or not. Note: this feature was added by PR in 0.2.7, but has been changed to return an `FileInclusionModes` enum for more explicit options rather than the original `bool`.
